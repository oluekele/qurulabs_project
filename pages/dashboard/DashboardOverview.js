...
import House from '@/data/BuildingPaymentData'
...
const DashboardPage = () => {
  ...
  const [bgCol, setBgCol] = useState({backgroundColor: 'gray-300'})
  const outerCircle = useRef()
  const handleChange  =()=>{
    console.log(outerCircle.current)
  } 
  // build.map((item)=>{
  //   outerCircle.current.style.backgroundColor = red
  //   console.log(item.paid)
  // })
  
  // const outer = document.getElementsByClassName('outter');
  // outer.BgImage = 'red'
  // outerCircle.current.style.backgroundColor = "red"
            ...
            <div className='flex overflow-hidden items-center gap-8'>
              {build.map((item)=>(
                <>
                  ...
                  <div key={item.id} className='border-[0.5px] rounded-[20px]  w-[330px] py-5  '>
                  <div className='flex items-center px-8  gap-4'>
                    <div className=' bg-white rounded-full  relative' >
                      <div ref={outerCircle} className="outter "
                        onLoad={handleChange}
                      ><div className="inner "><div id="number1" className=''></div></div></div>
